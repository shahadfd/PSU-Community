...
class UserApi {
    ...
    func observeUserByUsername(userFname: String, completion: @escaping (UserModel) -> Void) {
        REF_USERS.queryOrdered(byChild: "userFname_lowercase").queryEqual(toValue: userFname).observeSingleEvent(of: .childAdded, with: {
            snapshot in
